...
code before investing in the other four executives, desktop/mobile shells,
and the remaining ~25 modules.

## What's real here

- **Postgres** (`database/migrations/001_init.sql`) — real schema: `organizations`,
...
- **Python AI service** (`services/ai`, FastAPI) — the `AIProvider`
  ...
  `model_router` stub (section 66), and one fully wired agent: **cmo_agent**.
  Internal API is protected by a shared-secret header so only the Node
  gateway can call it (section 41: agents are privileged components, not
  public endpoints).
...
- **React + TypeScript frontend** (`apps/web`) — real login/register flow,
  a chat interface for cmo_agent with streaming-ready plumbing (currently
...
- **Only `cmo_agent` is wired.** `ceo_agent`, `coo_agent`, `cto_agent`,
  `cfo_agent` are not implemented — the point of this slice was proving one
  agent works end to end through every layer, not shipping five shallow ones.
  Adding another agent is: a new `agents/<key>/system_prompt.md`, a new
  `app/agents/<key>_agent.py` runner (copy `cmo_agent.py`), a row in the
  `agents` table, and a route in `services/ai/app/main.py`. No architectural
  change required — this was one of the spec's hard requirements (section 70:
  future agents must plug in without a rewrite).
...
- **No orchestrator, no War Room, no multi-agent routing** (sections 18–21) —
  those only make sense once there's more than one agent to route between.
...
`demo-org` (seeded by the migration), and chat with SHADOW CMO. Try: *"Score
this lead: Acme Corp. Their LinkedIn post says: we're hiring, our website
still says coming soon, and we're looking for a marketing agency."*

## Suggested next slices (in priority order)

1. **Second agent (`cfo_agent`)** — proves the "no rewrite to add an agent"
   claim with a second real example, and unlocks the simplest cross-agent
   workflow from the spec (section 55: revenue-below-target automation).
2. **shadow_orchestrator + routing engine** (sections 18–19) — only useful
   once ≥2 agents exist; routes one user request to multiple agents and
   aggregates.
...
├── services/ai/         Python AI service (FastAPI) + cmo_agent
